...
测试版“帮助 → 关于本站”是面向个人玩家的上手指南：按“输入具体场景、观察角色接戏、续写加料、故事卡回退、理解重玩乐趣、保留导演权”的顺序，以标题和正文两级连续排版，并内嵌压缩后的 `02f｜早点走进会议室` WebP 宣传图。首屏只显示一个可点击角色图标，点击轮换叫叫、铃铛、猪小弟；Web Animations API 直接对图标的 10 层完整副本做切片撕裂，减少动态效果时自动回退为静态。测试版还按需加载 `uisfx` 的 `cinematic` 音色，为可点击控件提供语义化反馈；音效只在真实交互后启动，可在“显示 → 界面音效”或关于页右上角关闭，并用 `localStorage: ququ:beta-ui-sfx` 记忆偏好。正式微信与正式钉钉不启用这套玩家指南和全局音效。
