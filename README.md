...
- 实验室可选择智谱 GLM-4.7-Flash、豆包 Seed-2.0-mini、DeepSeek V4 Flash 或自定义模型；默认使用免费的智谱并记住本机选择，只有选择自定义模型时才显示接口输入框。
...
智谱 `GLM-4.5-Flash` 已下线并自动迁移到免费的 `GLM-4.7-Flash`，所以项目直接使用当前模型 ID `glm-4.7-flash`。豆包使用火山方舟模型 ID `doubao-seed-2-0-mini-260215`。本地全栈模式通过 `/api/story/continue` 代理请求，部署环境同样从服务端环境变量读取密钥；火山方舟还兼容 `ARK_API_KEY` 别名。

智谱免费模型拥堵时会原样提示服务商错误，不会自动切换到可能产生费用的豆包；用户可在实验室中手动切换。
