...
测试版钉钉“帮助 → 关于本站”是黑客松介绍页：集中展示一句话生成、多角色调度、故事卡续写/回滚和钉钉沉浸感，并内嵌压缩后的 `02f｜早点走进会议室` WebP 宣传图。叫叫、铃铛、猪小弟的头像和中英文姓名可点击轮换，切换时由 Web Animations API 驱动整块徽章切片撕裂；减少动态效果时自动回退为静态。测试版还按需加载 `uisfx` 的 `cinematic` 音色，为可点击控件提供语义化反馈；音效只在真实交互后启动，可在“显示 → 界面音效”或关于页右上角关闭，并用 `localStorage: ququ:beta-ui-sfx` 记忆偏好。正式微信与正式钉钉不启用这套黑客松介绍和全局音效。
